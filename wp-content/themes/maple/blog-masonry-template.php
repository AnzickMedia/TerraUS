...
get_header(); maple_print_menu();
	...
	if (get_post_meta($maple_thisPostID, "maple_enable_custom_pagetitle_options_value", true) == "no" || !get_post_meta($maple_thisPostID, "maple_enable_custom_pagetitle_options_value", true)){
		...
		//$height = get_option("maple_header_height"); 
	...
	if ($type != "without"){
		...
		<div class="fullwidth-container <?php if ($type == "pattern") echo "bg-pattern"; ?> <?php if ($pt_parallax) echo "parallax"; ?>" <?php if ($pt_parallax) echo 'data-stellar-ratio="0.5"'; ?> style="
	    	<?php
		    	$maple_output = "";
		 		if ($height != "") $maple_output.= "height: ". $height . ";";
				if ($type == "none") $maple_output.= "background: none;"; 
				if ($type == "color") $maple_output.= "background: " . $color . ";";
				if ($type == "image") $maple_output.= "background: url(" . $image . ") no-repeat; background-size: 100% auto;";  
	 			if ($type == "pattern") $maple_output.= "background: url('" . $pattern . "') 0 0 repeat;";
		 			
	 			if (defined("MAPLE_IS_FIRST_PAGE")) echo "margin-top:60px;";
				echo esc_attr($maple_output);
	    	...
	    	<?php
		    	if ($type == "image" && $pt_overlay){
			    	...
			    	if ($pt_overlay_type == "color") $maple_output.= 'background-color:'.$pt_overlay_color;
			    	else $maple_output.= 'background:url('.$pt_overlay_pattern.') repeat;opacity:'.$pt_overlay_opacity.';';
			    	echo esc_attr($maple_output).'"></div>';
		    	}
		    	if ($type === "banner"){
			    	?> 
			    	<div class="revBanner">
					...
					<?php
						if ($showtitle){
							...
							<h1 class="page_title" style="<?php echo esc_attr("color: #$tcolor; font-size: $tsize; font-family: '{$principalfont[0]}';font-weight: {$principalfont[1]};");?><?php if ($margintop != "") echo esc_attr("margin-top: ".intval($margintop,10)."px;"); ?>">
								<?php if (is_front_page() && is_home()) echo wp_kses_post(get_option('maple_blog_single_primary_title')); else echo wp_kses_post(get_the_title($maple_thisPostID)); ?>
							</h1>
							<?php
						}
		    			if ($showsectitle){
			    			if (get_post_meta($post->ID, 'secondaryTitle_value', true) != "" || get_option('maple_blog_secondary_title') != ""){
						    	?>
	...
	$sidebar = get_post_meta($maple_thisPostID, 'sidebars_available_value', true);
	if (!$sidebar) $sidebar = "defaultblogsidebar";
	switch ($sidebar_scheme){
		case "none":
			if (!defined("MAPLE_IS_FIRST_PAGE")){
				?>
				<div class="content-before-blog">
					<?php 
						maple_content_shortcoder($post->post_content);
	...
	function maple_print_masonry_blog(){
	    ...
	    global $post, $wp_query, $maple_the_query;
	    
	    $maple_the_query = new WP_Query( $args );
	    				    
	    if ($maple_the_query->have_posts()){ 
	    
	    	if (get_post_meta(get_the_ID(), 'posts_add_category_filter_value', true) == 'yes' && count($categories) > 2){
		    	?>
		    	<ul class="posts_category_filter">
			    	<li class="showall" onclick="window.iso.isotope({filter:'*'});"><?php esc_html_e('All','maple'); if (get_post_meta(get_the_ID(), 'posts_add_category_filter_counter_value', true) == 'yes'){ ?>
	    	...
	    	<?php
			    ...
			    while ($maple_the_query->have_posts()){
				    $maple_the_query->the_post();
				    $postid = get_the_ID();
			...
			<?php
				...
				wp_reset_postdata();
				if ($maple_reading_option != "paged" && $maple_reading_option != "dropdown"){ 
					if (function_exists('icl_t')){
						next_posts_link( '<div class="next-posts"><i class="fa fa-angle-left"></i>'.sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Previous posts', get_option('maple_previous_text'))).'</div>', $maple_the_query->max_num_pages);  
						previous_posts_link('<div class="prev-posts">'.sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Next posts', get_option('maple_next_text'))).'<i class="fa fa-angle-right"></i></div>', $maple_the_query->max_num_pages);
					} else {
						next_posts_link( '<div class="next-posts"><i class="fa fa-angle-left"></i>'.sprintf(esc_html__("%s", "maple"), get_option("maple_previous_text")).'</div>', $maple_the_query->max_num_pages);  
						previous_posts_link('<div class="prev-posts">'.sprintf(esc_html__("%s", "maple"), get_option("maple_next_text")).'<i class="fa fa-angle-right"></i></div>', $maple_the_query->max_num_pages);
					}
				} else {
					maple_wp_pagenavi();
				}
