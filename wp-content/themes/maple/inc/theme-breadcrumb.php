<?php
function maple_the_breadcrumb(){
	$delimiter = $delimiter1 = "";
    $main = sprintf(esc_html__("%s", 'maple'), get_option('maple_breadcrumbs_home_text'));
	if (function_exists('icl_t')){
		$main = sprintf(esc_html__("%s", 'maple'), icl_t( 'maple', 'Home', get_option('maple_breadcrumbs_home_text')));
	}
    $maxLength= 30;
    ...
    if (!is_front_page()) {         
        ...
        elseif ( is_search() ) {  
            echo "<span>".esc_html__('Search Results for "', 'maple') . get_search_query() . '"</span>';
        }       
        ...
        elseif ( is_author() ) {
            ...
            echo  "<span>".esc_html__('Author&#39;s Article(s) ', 'maple') . wp_kses_post($delimiter . $user_info->display_name)."</span>" ;
        }       
        elseif ( is_404() ) {
