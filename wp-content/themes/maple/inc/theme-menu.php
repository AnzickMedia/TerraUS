...
function maple_menu_item_class_select(){
    global $pagenow;
	...
	$maple_inline_script = '
	    ...
	    function maple_override_class_options(){
		    ...
		    holders.each(function(){
			    ...
			    jQuery(this).prepend("<label class=\'maple_input\' for=\'maple_mega_menu\'>Mega Menu?  </label><input type=\'checkbox\' name=\'maple_mega_menu\'><label class=\'maple_input\' for=\'maple_mega_hide_title\'>Hide Title?  </label><input type=\'checkbox\' name=\'maple_mega_hide_title\'><br/><label class=\'maple_input\' for=\'maple_mega_hide_link\'>Just Label (Without Link) ?  </label><input type=\'checkbox\' name=\'maple_mega_hide_link\'><br/><a href=\'#\' class=\'maple_select_icon button\' >Select Icon</a><br/><br/>");
			    ...
			    if (jQuery(this).find("input.edit-menu-item-classes").val()){
				    ...
				    for (var i=0; i<itemClasses.length; i++){
					    ...
					    if (itemClasses[i].indexOf("maple_mega_hide_link") > -1) jQuery(this).find("input[name=\'maple_mega_hide_link\']").attr("checked","checked");
					    if (itemClasses[i].indexOf("maple_mega_hide_title") > -1) jQuery(this).find("input[name=\'maple_mega_hide_title\']").attr("checked","checked");
					    if (itemClasses[i].indexOf("maple_mega_menu") > -1) jQuery(this).find("input[name=\'maple_mega_menu\']").attr("checked","checked");
				    }
				    if (found != "false"){
					    jQuery(this).find(".maple_select_icon.button").after( "<i class=\'fa "+found+"\' style=\'position:relative;top:2px;margin-left:10px;margin-right:10px;width: 30px;height: 30px;font-size: 25px;border: 1px solid;text-align: center;line-height: 30px;\'></i><a class=\'des_remove_icon\' href=\'javascript:;\' onclick=\'maple_removeIcon(jQuery(this));\'>Remove Icon</a>" );
				    }
			    }
				...
				jQuery(this).find(".maple_select_icon").click(function(){
					jQuery(".maple_icon_container").dialog({modal:true, height: parseInt(jQuery(window).height()*.8, 10), width: parseInt(jQuery(window).width()*.8, 10), autoOpen: false});
					jQuery(".maple_icon_container").parent().attr("data-rel",theid).css({position : "fixed"}).end().dialog("open");
				});
		    });			
		}
			...
			jQuery(".submit-add-to-menu").click(function(){ 
				...
				if (new_item){
					var check_for_new_item = setInterval(function(){
						if (jQuery(".menu-item-settings p.field-css-classes label").not(".des").not(".maple_input").length){
	...
	';
	wp_add_inline_script('maple-admin', $maple_inline_script, 'after');
	?>
	
	<div class="maple_icon_container">
		<?php
			$first = true;
		...
		<a href="javascript:;" onclick=" if (jQuery('#'+jQuery(this).closest('.ui-dialog').attr('data-rel')+ ' .des_remove_icon').length) jQuery('#'+jQuery(this).closest('.ui-dialog').attr('data-rel')+ ' .des_remove_icon').click(); jQuery('#'+jQuery(this).closest('.ui-dialog').attr('data-rel')+ ' .edit-menu-item-classes').val( jQuery('#'+jQuery(this).closest('.ui-dialog').attr('data-rel')+ ' .edit-menu-item-classes').val() + ' ' + jQuery(this).siblings('i.selected').attr('data-rel')); jQuery('#'+jQuery(this).closest('.ui-dialog').attr('data-rel')+ ' .maple_select_icon.button').after('<a class=\'des_remove_icon\' href=\'javascript:;\' onclick=\'maple_removeIcon(jQuery(this));\'>Remove Icon</a>').after( '<i class=\''+jQuery(this).siblings('i.selected').attr('data-rel')+'\' style=\'position:relative;top:2px;margin-left:10px;margin-right:10px;width: 30px;height: 30px;font-size: 25px;border: 1px solid;text-align: center;line-height: 30px;\'></i>' );   jQuery('.maple_icon_container').dialog('close');" class="button button_ok">OK</a>
		<a href="javascript:;" onclick="jQuery('.maple_icon_container').dialog('close');" class="button button_cancel">Cancel</a>
	</div>
    <?php
    }
}
add_action('admin_footer','maple_menu_item_class_select');
