 ...
 */
class MapleStyleOptionsManager{
	...
	/**
	 * The main constructor for the MapleStyleOptionsManager class
	...
	function print_heading($heading_text){
		...
		echo '<div id="maple-content-container"><form method="post" id="maple-style-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('maple-theme-update-style-options','maple-theme-style-options');
		}
	...
	function print_upload($value){
		...
		$maple_admin_inline_script .= '
			jQuery(document).ready(function(){
				maple_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->maple_utils_url).'upload-handler.php", "'.esc_js($this->maple_uploads_url).'");
			});
		';
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<li onclick="jQuery(this).parents(\'.sub-navigation-container\').find(\'#'."maple".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'.sub-navigation-container\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_custom($value){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<script type="text/javascript">jQuery(document).ready(function($){ "use strict";
								maple_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->maple_utils_url).'upload-handler.php", "'.esc_js($this->maple_uploads_url).'");
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				"use strict";
				maple_StyleOptionsManager.setCustomFieldsFunc("'.esc_js($value['id']).'", ["'.esc_js($idsString).'"], ["'.esc_js($namesString).'"], ['.esc_js($textareaString).'] , "'.esc_js($prv).'",  "");
				jQuery(\'#maple_export_style_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().click(function(){
