...
class MapleOptionsManager{
	...
	var $maple_images_url='';
	var $maple_utils_url='';
	var $maple_uploads_url='';
	var $maple_version='';
	var $themename='';
	var $first_save='';
	
	/**
	 * The main constructor for the MapleOptionsManager class
	...
	function __construct($themename, $images_url, $utils_url, $uploads_url, $version){
		...
		$this->maple_images_url=$images_url;
		$this->maple_utils_url=$utils_url;
		$this->maple_uploads_url=$uploads_url;
		$this->maple_version=$version;
		$this->first_save=get_option("maple_first_save");
	}
	...
	function print_heading($heading_text){
		...
		if(isset($_GET['activated'])&&$_GET['activated']=='true'){
			...
			$opt = get_option('maple_enable_website_loader');
			if (!is_string($opt)) {
				echo '<iframe style="display:none;" src="'.esc_url(get_admin_url()).'admin.php?page=maple_options"></iframe>';
			}
			$sopt = get_option('maple_style_color');
			if (!is_string($sopt)) {
				echo '<iframe style="display:none;" src="'.esc_url(get_admin_url()).'admin.php?page=maple_style_options"></iframe>';
			}
		...
		echo '<div id="maple-content-container"><form method="post" id="maple-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('maple-theme-update-options','maple-theme-options');
		}
		echo '<div id="sidebar"><h3 id="theme_name">'.esc_html($this->themename).' <span>v.'.esc_html($this->maple_version).'</span></h3><div id="navigation"><ul>';
		...
		foreach ($this->options as $value) {
			...
			if($value['type']=='title'){
				...
				echo '<li><span><a href="#navigation-'.esc_attr($i).'"><div class="'.esc_attr(strtolower(str_replace(" ", "_", $value['name']))).'"></div><span>'.esc_html($value['name']).'</span></a></span></li>';
				$i++;
			}
		}
	...
	function print_footer(){
		echo '</div></div><div id="maple-footer"><input type="hidden" name="action" value="save" />
	...
	function print_textarea($value){
		...
		$input_value = $this->get_field_value($value['id'], $std);
		if (isset($value['params']) && $value['params'] == 'maple_rich_editor'){
...
jQuery("#'.esc_js($value['id']).'").html( jQuery("iframe#'.esc_js($value['id']).'_editor_ifr").contents().find("#tinymce.'.esc_js($value['id']).'_editor" ).html() ) });
	...
	function print_upload($value){
		...
		echo '<div id="'.esc_attr($value['id']).'_button" class="upload-button upload-logo" ><a class="maple-button alignright"><span class="upload-panel">Upload</span></a></div><br/>';
		$maple_admin_inline_script = (isset($maple_admin_inline_script)) ? $maple_admin_inline_script : "";
		$maple_admin_inline_script .= '
			jQuery(document).ready(function(){
				mapleOptions.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->maple_utils_url).'upload-handler.php", "'.esc_js($this->maple_uploads_url).'");
			});
		';
		wp_add_inline_script('maple-admin', $maple_admin_inline_script, 'after');
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			if($type=='pattern') {
				...
				if($option != "none")
					$style='background-image:url('.MAPLE_PATTERNS_URL.$option.');';
				else
			...
			echo '<li onclick="jQuery(this).parents(\'#tab_navigation-2-general\').find(\'#'."maple".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'#tab_navigation-2-general\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_website_loaders($value){
		...
		foreach ($value['options'] as $option) {
			...
			if ($class == "load2" || $class == "load3" || $class == "load6"){
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."maple".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box '.esc_attr($class).'"><div class="loader"></div></div></div>';
			} else {
				echo '<div onclick="jQuery(this).closest(\'.loaders-styles-holder\').find(\'#'."maple".'_website_loader\').attr(\'value\',\''.esc_js($option['id']).'\'); jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');" class="loader-item '.esc_attr($selected).'"><div class="loaders-style-box loader-inner '.esc_attr($class).'">'.wp_kses_post($divs).'</div></div>';
			}
...
*/
	...
	function print_custom($value){
		...
		foreach($value['fields'] as $field){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					echo '<div id="'.esc_attr($field['id']).'_button" class="upload-button upload-logo" ><a class="maple-button alignright"><span>Upload</span></a></div><br/>';
					echo '<script type="text/javascript">jQuery(document).ready(function($){ "use strict";
								mapleOptions.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->maple_utils_url).'upload-handler.php", "'.esc_js($this->maple_uploads_url).'");
						});</script>';
					$preview=$field['id'];
					$is_textarea[]="false";
				break;
			...
			if ($field['id'] == "maple_google_fonts_name"){
				...
				if (count($fonts) > 1){
					...
					foreach($fonts as $f){
						if ($f != "")
							echo 'jQuery(\'#add_google_font_list\').append(\'<li><b>Font URL: </b><span class="maple_google_fonts_name_span">'.$f.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';
					}
					echo '});
					</script>';	
				}
			...
			if ($field['id'] == "maple_sidebar_name_name"){
				...
				if (count($sidebars) > 0){
					...
					foreach($sidebars as $s){
						if ($s != ""){
							echo 'jQuery(\'#sidebar_name_list\').append(\'<li><b>Name: </b><span class="maple_sidebar_name_span">'.$s.'</span><br><div class="editButton hover"></div><div class="deleteButton hover"></div></li>\');';	
						}
					}
					echo '});
					</script>';	
		...
		echo '<a class="maple-button custom-option-button" id="'.esc_attr($value['id']).'_button"><span>'.wp_kses_post($value['button_text']).'</span></a>';
		
		//print the list that will contain the added items
		//if ($value['id'] != 'maple_reset_options_button')
		...
		echo '<script type="text/javascript">
			jQuery(document).ready(function($){
				"use strict";
				mapleOptions.setCustomFieldsFunc("'.esc_js($value['id']).'", ["'.esc_js($idsString).'"], ["'.esc_js($namesString).'"], ['.esc_js($textareaString).'] , "'.esc_js($prv).'",  "");
				jQuery(\'#maple_export_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().click(function(){
	...
	function print_mediauploader($value){
		$modal_update_href = esc_url( add_query_arg( array(
		    'page' => 'maple_gallery',
		    '_wpnonce' => wp_create_nonce('maple_gallery_options'),
	...
	function print_insta_authorize($value){
		...
		$admin_url = admin_url('themes.php?page=maple_options');
		$theme = "maple";
		...
		if (isset($_GET['deauthorize_insta']) && $_GET['deauthorize_insta'] != false){
			...
			if (isset($_GET['access_token']) || $input_value != ""){
				...
				if (isset($_GET['access_token'])){
					...
					update_option( 'maple_insta_token', $input_value );
				}
	...
	function validate_response($json = null) {
		...
		if (!($response = json_decode(wp_remote_retrieve_body($json), true)) || 200 !== wp_remote_retrieve_response_code($json)) {
			...
			if (is_wp_error($json)) {
				...
				$response = array(
					'error' => 1,
					'message' => esc_html__('Unknown error occurred, please try again', 'maple')
				);
			}
		}
		return $response;
