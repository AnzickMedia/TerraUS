...
add_action('admin_menu', 'maple_add_custom_pages');
add_action('wp_ajax_maple_insert_post', 'maple_insert_post');
add_action('wp_ajax_maple_add_instance', 'maple_add_instance');
add_action('wp_ajax_maple_save_order', 'maple_save_order');
add_action('wp_ajax_maple_detele_item', 'maple_detele_item');
add_action('wp_ajax_maple_edit_item', 'maple_edit_item');
add_action('wp_ajax_maple_detele_instance', 'maple_detele_instance');



//define the main constants that will be used
define("MAPLE_CUSTOM_PREFIX", 'custom_');
define("MAPLE_DEFAULT_TERM", 'default');
define("MAPLE_TERM_SUFFIX", '_category');
define("MAPLE_NONCE", 'maple-custom-page');

$maple_data->custom_pages=array();
/**
 * Adds all the custom pages to the menu.
 */
function maple_add_custom_pages(){
	global $maple_data;

	foreach($maple_data->custom_pages as $page){
		$portfolio_permalink = get_option("maple_portfolio_permalink");
		add_theme_page( 'edit.php?post_type='.$portfolio_permalink, $page->page_name, 'delete_pages', $page->post_type, 'maple_build_custom_page' );		
	}
}


function maple_get_created_camera_sliders(){
	$maple_slider_data=array();
	global $wpdb;
	...
	if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
		...
		$maple_slider_data = array(array('id'=>'no_slider','name'=>'No Sliders Found.'));
	} else {
		...
		if (count($revsliders)>0){
			...
			$maple_slider_data = array_merge($maple_slider_data,$revsliders);
		} else {
			$maple_slider_data = array(array('id'=>'no_slider','name'=>'No Sliders Found.'));
		}
	}
	...
	if($wpdb->get_var( $wpdb->prepare("SHOW TABLES LIKE %s", $table_name) ) != $table_name) {
		...
		if (count($mastersliders)>0){
			...
			$maple_slider_data = array_merge($maple_slider_data,$mastersliders);
		}
	}

	if (count($maple_slider_data) < 1) $maple_slider_data = array(array('id'=>'no_slider','name'=>'No Sliders Found.'));
	return $maple_slider_data;
}
 ...
 */
function maple_build_custom_page(){
	if(isset($_GET['page'])){
		global $maple_data;

		$pageid=$_GET['page'];
		$custom_page=$maple_data->custom_pages[$pageid];
		$custom_page_manager=new MapleCustomPageManager($custom_page, MAPLE_CUSTOM_PREFIX, MAPLE_TERM_SUFFIX, MAPLE_DEFAULT_TERM, MAPLE_NONCE);
 ...
 */
function maple_insert_post(){
	//check the nonce field for security
	check_ajax_referer(MAPLE_NONCE, 'nonce');

	global $maple_data, $current_user;

	$post_type=$_POST['post_type'];
	$custom_page=$maple_data->custom_pages[$post_type];

	//insert the post
	$dataManager=new MapleCustomDataManager();
	$post=$dataManager->insert_post($_POST, $custom_page, MAPLE_CUSTOM_PREFIX, MAPLE_TERM_SUFFIX);

	//get the display template for the inserted post
	$templater=new MapleTemplater();
	echo wp_kses_post($templater->get_custom_page_list_template($post, $custom_page, MAPLE_CUSTOM_PREFIX));
	die();
...
function maple_add_instance(){
	...
	$custom_page=$maple_data->custom_pages[$_POST['post_type']];
	...
	if($res instanceof WP_Error){
		...
		$html.=$templater->get_custom_page_form_template($_POST['name'], $res['term_id'], $custom_page, MAPLE_CUSTOM_PREFIX);
 ...
 */
function maple_save_order(){
	//check the nonce field for security
	check_ajax_referer(MAPLE_NONCE, 'nonce');

	if(isset($_POST['order'])&& $_POST['order'] && isset($_POST['category']) && $_POST['category'] && isset($_POST['posttype'])){
			update_option('maple_order'.$_POST['category'].$_POST['posttype'], $_POST['order']);
	}
}
 ...
 */
function maple_get_ordered_post_list($posts, $category, $posttype){
	$new_post_array=array();

	$order=explode(',',get_option('maple_order'.$category.$posttype));
...
function maple_detele_item(){
	...
	if(isset($_POST['itemid']) && isset($_POST['category']) && isset($_POST['posttype'])){
		...
		if($res){
			...
			$order_option='maple_order'.$_POST['category'].$_POST['posttype'];
			$order_arr=explode(',',get_option($order_option));
			$new_order=maple_remove_item_by_value($order_arr,$_POST['itemid']);
 ...
 */
function maple_edit_item(){
	//check the nonce field for security
	check_ajax_referer(MAPLE_NONCE, 'nonce');

	if(isset($_POST['itemid'])&& $_POST['itemid']){
		$dataManager=new MapleCustomDataManager();
		$post=$dataManager->edit_post($_POST, MAPLE_CUSTOM_PREFIX);
	}
}
 ...
 */
function maple_detele_instance(){
	//check the nonce field for security
	check_ajax_referer(MAPLE_NONCE, 'nonce');
