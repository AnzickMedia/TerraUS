...
if ( isset($_GET['page']) && $_GET['page'] == MAPLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'maple_init_options_functionality');  
	add_action('init', 'maple_set_options'); 
}

if ( isset($_GET['page']) && $_GET['page'] == MAPLE_STYLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'maple_init_style_options_functionality');  
	add_action('init', 'maple_set_style_options'); 
}
 ...
 */
function maple_init_style_options_functionality(){
	global $maple_style_options;

	$maple_style_options=array();

	//get all the categories
	$categories=get_categories('hide_empty=0');
	$maple_categories=array();
	for($i=0; $i<sizeof($categories); $i++){
		$maple_categories[]=array('id'=>$categories[$i]->cat_ID, 'name'=>$categories[$i]->cat_name);
	}

	//load the files that contain the options
	$maple_style_options_files=array('style-general', 'style-website-loading', 'style-header-top-bar', 'style-logotype', 'style-menu', 'style-sliding-panel', 'style-search', 'style-pagetitle', 'style-footer', 'style-typography', 'style-import-export');
	foreach($maple_style_options_files as $file){
		require_once(MAPLE_OPTIONS_PATH.$file.'.php');
	}
}
...
function maple_set_style_options(){
	...
	if ( $_GET['page'] == MAPLE_STYLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['maple-theme-style-options'],'maple-theme-update-style-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("maple_style_first_save")){
						update_option("maple_style_first_save",'true');
					}
					...
					$doc->save($filename);		
					header("Location: themes.php?page=".MAPLE_STYLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 * to populate the global options array.
 */
function maple_init_options_functionality(){
	global $maple_options;
	...
	//load the files that contain the options
	$maple_options_files=array('general', 'header-menu', 'widgetsarea', 'sliders', 'translation', 'social', 'customcss', 'data');
	
	foreach($maple_options_files as $file){
		require_once(MAPLE_OPTIONS_PATH.$file.'.php');
	}
}
...
function maple_set_options(){
	...
	//insert the default values if the fields are empty
	foreach ($maple_options as $value) {
		if(isset($value['id'])){
			if(get_option($value['id'])=='' && isset($value['std']) && !in_array($value['type'], $nonsavable_types)){
				update_option( $value['id'], $value['std']);
			}
	...
	//save the field's values if the Save action is present
	if ( $_GET['page'] == MAPLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['maple-theme-options'],'maple-theme-update-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("maple_first_save")){
						update_option("maple_first_save",'true');
					}
					...
					foreach ($maple_options as $value) {
						...
						if (isset($value['id'])){
							if( isset( $_REQUEST[ $value['id'] ] ) && !in_array($value['type'], $nonsavable_types)) {
								...
								if ($value['id'] === "maple_portfolio_permalink"){
									$oldval = get_option("maple_portfolio_permalink");
					...
					$doc->appendChild($root);
					$doc->save($filename);		
					header("Location: themes.php?page=".MAPLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 */
function maple_theme_admin() {
	global $maple_options,$maple_options_manager;

	$maple_options_manager=new MapleOptionsManager("Maple", MAPLE_IMAGES_URL, MAPLE_UTILS_URL, MAPLE_UPLOADS_URL, MAPLE_VERSION);
	$maple_options_manager->set_options($maple_options);

	if ( isset($_REQUEST['saved'] )) {
		$maple_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$maple_options_manager->print_reset_message();
	}

	$maple_options_manager->print_heading("");
	$maple_options_manager->print_options();
	$maple_options_manager->print_footer();
}
 ...
 */
function maple_theme_style_options_admin() {
	global $maple_style_options, $maple_style_options_manager;

	$maple_style_options_manager=new MapleStyleOptionsManager("Maple", MAPLE_IMAGES_URL, MAPLE_UTILS_URL, MAPLE_UPLOADS_URL, MAPLE_VERSION);
	$maple_style_options_manager->set_options($maple_style_options);

	if ( isset($_REQUEST['saved'] )) {
		$maple_style_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$maple_style_options_manager->print_reset_message();
	}

	$maple_style_options_manager->print_heading("");
	$maple_style_options_manager->print_options();
	$maple_style_options_manager->print_footer();
}
 ...
 */
function maple_theme_demos_admin() {
	global $maple_demos_manager;

	$maple_demos_manager=new MapleDemosManager("Maple", MAPLE_IMAGES_URL, MAPLE_UTILS_URL, MAPLE_UPLOADS_URL, MAPLE_VERSION);

	$maple_demos_manager->print_heading("");
	$maple_demos_manager->print_options();
	$maple_demos_manager->print_footer();
}
 ...
 */
function maple_add_options($option_arr){
	global $maple_options;

	foreach($option_arr as $option){
		$maple_options[]=$option;
	}
}

function maple_add_style_options($option_arr){
	global $maple_style_options;

	foreach($option_arr as $option){
		$maple_style_options[]=$option;
	}
}
 ...
 */
function maple_get_opt($option){
	return stripslashes(get_option("maple".$option));
}


function maple_get_custom_sidebars(){
	$res=array();
	$sides=get_option('maple_sidebar_name_names');
	if($sides){
		$res=explode(MAPLE_SEPARATOR, $sides);
		array_pop($res);
	}
	return $res;
}
 ...
 */
function maple_add_custom_options($opt_array){
	$new_maple_options=array();
	...
	foreach($opt_array as $option){
		if($option['type']=='multiple_custom'){
			...
			$saved_array=explode(MAPLE_SEPARATOR, $saved_values);
			if(sizeof($saved_array)>1){
				array_pop($saved_array);
				foreach($saved_array as $custom_name){
					$id=maple_convert_to_class($custom_name);
					$custom_option=array(
					"id"=>$id,
					...
					array_push($new_maple_options, $custom_option);
				}
			}
				
		} else{
			//this is just a normal option, just append it into the new array
			array_push($new_maple_options, $option);
		}
	}

	return $new_maple_options;
}

function maple_fonts_array_builder(){
	$defaultfonts = array();
...
function maple_portfolio_types(){
...
function maple_get_proj(){
	
	$maple_proj2=array(array('id'=>'default', 'name'=>'Default'));
	if (function_exists('maple_get_projects')){
		$maple_proj1 = maple_get_projects();
		foreach($maple_proj1 as $dp){
			$maple_proj2[]=array("name"=>$dp['p_title'], "id"=>$dp['p_id']);
		}
	}
...
function maple_get_all_google_fonts(){
	$maple_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
