 ...
 * ADD THE ACTIONS
 */
add_action('admin_menu', 'maple_load_meta_boxes');
add_action('admin_menu', 'maple_create_meta_box');  
add_action('admin_menu', 'maple_create_meta_post_box');  
add_action('admin_menu', 'maple_create_meta_portfolio_box'); 
add_action('admin_menu', 'maple_create_meta_testimonials_box');  
add_action('save_post', 'maple_save_postdata');  
add_action('save_post', 'maple_save_post_postdata');  
add_action('save_post', 'maple_save_portfolio_postdata'); 
add_action('save_post', 'maple_save_testimonials_postdata');
add_action('admin_footer','maple_print_helper');

if (!defined('MAPLE_PORTFOLIO_POST_TYPE')){
	$portfolio_permalink = get_option("maple_portfolio_permalink");
	if (!get_option("maple_portfolio_permalink")) define("MAPLE_PORTFOLIO_POST_TYPE", "portfolio");
	else define("MAPLE_PORTFOLIO_POST_TYPE", get_option("maple_portfolio_permalink"));
}
...
if (isset($_REQUEST['file'])) { 
    //check_admin_referer("maple_gallery_options");
    $options = get_option('maple_gallery_options', TRUE);
	if (isset($options['default_image'])) $options['default_image'] = absint($_REQUEST['file']) ? absint($_REQUEST['file']) : "";
    update_option('maple_gallery_options', $options);
}


function maple_load_meta_boxes(){
	//load the porftfolio categeories
	$sidebar_taxonomies=maple_get_custom_sidebars();
	...
	foreach($sidebar_taxonomies as $taxonomy){
		$sidebar_categories[]=array("name"=>$taxonomy, "id"=>maple_convert_to_class($taxonomy));
	}
	
	//load the porftfolio categeories
	$portf_categories=array(array('id'=>'all', 'name'=>'All Portfolios'));
	if (function_exists('maple_get_taxonomies')){
		$portf_taxonomies=maple_get_taxonomies('portfolio_type');
	...
	global $maple_data, $maple_new_meta_boxes, $maple_new_meta_portfolio_boxes, $maple_new_meta_testimonials_boxes, $maple_new_meta_post_boxes;
		 ...
		 * ------------------------------------------------------------------------*/
		 ...
		 if (get_option("maple_body_type") == "pattern") $varType = MAPLE_PATTERNS_URL.get_option("maple_body_pattern");
		 else $varType =  get_option("maple_header_body_pattern");
	
		//the meta data for pages
		$maple_new_meta_boxes =
		array(
		...
		array(
			...
			"name" => "maple_enable_custom_header_options",
			"type" => "select",
			"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
			"std" => "no"
		),
		...
		array(
			...
			"name" => "maple_enable_website_loading",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."), array("id"=>"off", "name"=>"No, thanks.")),
			"std" => get_option('maple_enable_website_loader'),
			"description" => "If set to YES, you'll need to activate the 'Enable Website Loading' options in <strong>Appearance</strong> > <strong>Maple Options</strong> > <strong>General</strong> > <strong>Main Settings</strong> to choose the Loader Style and Save the options. You can then deactive the option again."
		),
		
		array(
			"title" => "Page Content Starts...",
			"name" => "maple_content_to_the_top",
			"type" => "select",
			"options" => array(array("id"=>"off","name"=>"After the header"), array("id"=>"on","name"=>"Behind the header")),
			"std" => get_option('maple_content_to_the_top')
		),
		
		array(
			"title" => "Header Pre Scroll Style",
			"name" => "maple_custom_header_pre",
			"type" => "select",
			"options" => array(array("id"=>"light","name"=>"Light"), array("id"=>"dark", "name"=>"Dark")),
			"std" => get_option('maple_header_style_light_dark') != "" ? get_option('maple_header_style_light_dark') : "light"
		),
		
		array(
			"title" => "Header After Scroll Style",
			"name" => "maple_custom_header_after",
			"type" => "select",
			"options" => array(array("id"=>"light","name"=>"Light"), array("id"=>"dark", "name"=>"Dark")),
			"std" => get_option('maple_header_after_scroll_style_light_dark') != "" ? get_option('maple_header_after_scroll_style_light_dark') : "light"
		),
		
		array(
			"title" => "Enable Custom Page Title Options ?",
			"name" => "maple_enable_custom_pagetitle_options",
			"type" => "select",
			"options" => array(array("id"=>"yes","name"=>"Yes, please."), array("id"=>"no", "name"=>"No, thanks.")),
			"std" => "no"
		),
		...
		array(
			...
			"std" => get_option('maple_header_type')
		),
		
		array(
			"title" => "Image",
			"name" => "maple_header_image",
			"type" => "mediaupload",
		...
		array(
			"title" => "Parallax ?",
			"name" => "maple_pagetitle_image_parallax",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."),array("id"=>"off","name"=>"No, thanks.")),
			"std" => "off",
		),
		...
		array(
			"title" => "Overlay ?",
			"name" => "maple_pagetitle_image_overlay",
			"type" => "select",
			"options" => array(array("id"=>"on","name"=>"Yes, please."),array("id"=>"off","name"=>"No, thanks.")),
			"std" => "off"
		),
		...
		array(
			"title" => "Overlay Type",
			"name" => "maple_pagetitle_overlay_type",
			"type" => "select",
		...
		array(
			"title" => "Overlay Color",
			"name" => "maple_pagetitle_overlay_color",
			"type" => "color",
			"std" => "212121"
		),
		...
		array(
			...
			"name" => "maple_pagetitle_overlay_pattern",
			"type" => "pattern",
			"options" => $patterns,
		),
		...
		array(
			...
			"name" => "maple_pagetitle_overlay_opacity",
			"type" => "slider",
			"std" => "100%"
		),
		// end of new options
		...
		array(
			"title" => "Color",
			"name" => "maple_header_color",
			"type" => "color"
		),
		
		array(
			"title" => "Color Opacity",
			"name" => "maple_header_color_opacity",
			"type" => "slider",
			"std" => "100"
		),
		...
		array(
			...
			"name" => "maple_header_custom_pattern",
			"type" => "mediaupload",
		...
		array(
			"title" => "Banner Slider",
			"name" => "maple_banner_slider",
			"type" => "select",
			"options" => maple_get_created_camera_sliders()
		),

		array(
			"title" => "Page Title Padding",
			"name"=> "maple_page_title_padding",
			"type" => "text",
			"std" => "140px"
		),
		...
		array(
			...
			"name" => "maple_header_text_alignment",
			"type" => "select",
			"std" => "center",
		...
		array(
			...
			"name" => "maple_header_text_font",
			"type" => "select",
			"options" => maple_fonts_array_builder(),
		...
		array(
			...
			"name" => "maple_header_text_margin_top",
			"type" => "text",
			"std" => "40px"
		),
		...
		array(
			...
			"name" => "maple_secondary_title_font",
			"type" => "select",
			"options" => maple_fonts_array_builder(),
			"description" => 'You can select one of the fonts that the theme goes with or you can add google fonts (Style Options > Fonts).',
			"std" => 'Helvetica Neue'
		...
		array(
			...
			"name" => "maple_secondary_title_text_color",
			"type" => "color",
			"std" => "949494"
		),
		...
		array(
			...
			"name" => "maple_secondary_title_text_size",
			"type" => "text",
			"std" => "19px"
		),
		...
		array(
			...
			"name" => "maple_header_secondary_text_margin_top",
			"type" => "text",
			"std" => "15px"
		),
		...
		array(
			...
			"name" => "maple_breadcrumbs_margin_top",
			"type" => "text",
			"std" => "30px"
		),
		...
		array(
			...
			array(
				...
				"std" => get_option("maple_projs_flex_navigation"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("maple_projs_flex_controls"),
				"description" => ""
			),
			...
			array(
				...
				"std" => get_option("maple_projs_flex_transition"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("maple_projs_flex_transition_duration"),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("maple_projs_flex_slide_duration"),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("maple_projs_flex_autoplay"),
				"description" => ""
			),
			...
			array(
				...
				"type" => "select",
				"options" => array(array("name"=>"Yes", "id"=>"true"), array("name"=>"No", "id"=>"false")),
				"std" => get_option("maple_projs_flex_pause_hover"),
			...
			array(
				...
				"type" => "text",
				"std" => get_option("maple_projs_flex_height"),
		...
		$maple_new_meta_testimonials_boxes =
		array(
 ...
 */
function maple_create_meta_box() {
	if ( function_exists('add_meta_box') ) {
		add_meta_box( 'new-meta-boxes', '<div class="icon-small"></div> '."Maple".' PAGE SETTINGS', 'maple_new_meta_boxes', 'page', 'normal', 'high' );
	}
}
 ...
 */
function maple_create_meta_post_box() {
	if ( function_exists('add_meta_box') ) {
		add_meta_box( 'new-meta-post-boxes', '<div class="icon-small"></div> '."Maple".' POST TYPE SETTINGS', 'maple_new_meta_post_boxes', 'post', 'normal', 'high' );
	}
}
 ...
 * Creates a post meta box.
 */
function maple_create_meta_portfolio_box() {
	if ( function_exists('add_meta_box') && defined('MAPLE_PORTFOLIO_POST_TYPE')) {
		add_meta_box( 'new-meta-portfolio-boxes', '<div class="icon-small"></div> '."Maple".' PORTFOLIO ITEM SETTINGS', 'maple_new_meta_portfolio_boxes', MAPLE_PORTFOLIO_POST_TYPE, 'normal', 'high' );
	}
}
 ...
 */
function maple_create_meta_testimonials_box() {
	if ( function_exists('add_meta_box') && defined('MAPLE_TESTIMONIALS_POST_TYPE')) {
		add_meta_box( 'new-meta-testimonials-boxes', '<div class="icon-small"></div> '."Maple".' TESTIMONIALS ITEM SETTINGS', 'maple_new_meta_testimonials_boxes', MAPLE_TESTIMONIALS_POST_TYPE, 'normal', 'high' );
	}
}
 ...
 */
function maple_new_meta_boxes() {
	global $post, $maple_new_meta_boxes;

	foreach($maple_new_meta_boxes as $meta_box) {
		maple_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function maple_new_meta_post_boxes() {
	global $post, $maple_new_meta_post_boxes;

	foreach($maple_new_meta_post_boxes as $meta_box) {
		maple_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function maple_new_meta_portfolio_boxes() {
	global $post, $maple_new_meta_portfolio_boxes;

	foreach($maple_new_meta_portfolio_boxes as $meta_box) {
		maple_print_meta_box($meta_box, $post);
	}
}
 ...
 * Calls the print method for portfolio meta boxes.
 */
function maple_new_meta_testimonials_boxes() {
	global $post, $maple_new_meta_testimonials_boxes;

	foreach($maple_new_meta_testimonials_boxes as $meta_box) {
		maple_print_meta_box($meta_box, $post);
	}
}
 ...
 */
function maple_print_meta_box($meta_box, $post){
	...
	switch($meta_box['type']){
		...
		case 'multicheck':
				...
				$maple_admin_inline_script .= '
					jQuery(document).ready(function(){
						...
						function update_multicheck_'.esc_js($mcindex).'(){
							...
							jQuery("#maple-multicheck-'.esc_js($mcindex).'").attr("value",multicheck_value);
							var allselected = true;
							for (var i = 0; i < jQuery("#multicheck-'.esc_js($mcindex).' > label").not(".button").length; i++){
								if (jQuery("#multicheck-'.esc_js($mcindex).' label").not(".button").eq(i).children("input").attr("checked") == undefined) allselected = false;
							}
			...
			echo'<input type="text" name="'.esc_attr($meta_box['name']).'_value" value="'.esc_attr($meta_box_value).'" id="maple-multicheck-'.esc_attr($mcindex).'" class="option-input hidden" />';
			echo '</div>';
			$mcindex++;
		break;
		case 'heading':
...
<h4>'.wp_kses_post($meta_box['title']).'</h4></div>';
		...
		case 'upload':
			...
			echo'<input type="text" name="'.esc_attr($meta_box['name']).'_value" value="'.esc_attr($meta_box_value).'" id="maple-'.esc_attr($meta_box['name']).'" class="option-input upload"/>';

			echo '<div id="maple-'.esc_attr($meta_box['name']).'_button" class="upload-button upload-logo" ><a class="button button-upload"><span>Upload</span></a></div><br/>';

			$maple_admin_inline_script = (isset($maple_admin_inline_script)) ? $maple_admin_inline_script : "";
			$maple_admin_inline_script .= '
				jQuery(document).ready(function(){
					"use strict";
					mapleOptions.loadUploader(jQuery("div#maple-'.esc_js($meta_box['name']).'_button"), "'.MAPLE_UTILS_URL.'upload-handler.php", "'.MAPLE_UPLOADS_URL.'");
				});
			';
		...
		case 'select':
			echo '<div class="option-container'; 
			if ($meta_box['name'] == 'maple_enable_custom_header_options' || $meta_box['name'] == 'maple_enable_custom_pagetitle_options') echo ' maple_folding_options ';
			echo '">';
			echo'<input type="hidden" name="'.esc_attr($meta_box['name']).'_noncename" id="'.esc_attr($meta_box['name']).'_noncename" value="'.wp_create_nonce( plugin_basename(__FILE__) ).'" />';
		...
		case 'pattern':
			...
			if(sizeof($meta_box['options'])>0){
				foreach ($meta_box['options'] as $option) { 
					if ($option['name'] != "none"){
						?> <div onclick="jQuery(this).addClass('selected').siblings().removeClass('selected');jQuery(this).parent().siblings('select').val('<?php echo esc_js($option['id']); ?>');" style="background-image:url(<?php echo esc_url(get_template_directory_uri()).'/images/maple_patterns/'.esc_attr($option['name']) ?>);" class="pattern_item
						<?php if ( $meta_box_value == $option['id']) {
							echo ' selected';
						}
						echo '"';
 ...
 */
function maple_save_postdata( $post_id ) {
	global $post, $maple_new_meta_boxes;

	if(get_post($post_id)->post_type=='page'){
		$maple_new_meta_boxes=$GLOBALS['maple_new_meta_boxes'];
		maple_save_meta_data($maple_new_meta_boxes, $post_id);
	}
}
 ...
 */
function maple_save_portfolio_postdata( $post_id ) {
	global $post, $maple_new_meta_portfolio_boxes;
	
	if(get_post($post_id)->post_type==MAPLE_PORTFOLIO_POST_TYPE){
		maple_save_meta_data($maple_new_meta_portfolio_boxes, $post_id);
	}
}
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function maple_save_testimonials_postdata( $post_id ) {
	global $post, $maple_new_meta_testimonials_boxes;
	
	if(get_post($post_id)->post_type==MAPLE_TESTIMONIALS_POST_TYPE){
		maple_save_meta_data($maple_new_meta_testimonials_boxes, $post_id);
	}
}
 ...
 * @param $post_id the ID of the post that contains the meta box
 */
function maple_save_post_postdata( $post_id ) {
	global $post, $maple_new_meta_post_boxes;

	if(get_post($post_id)->post_type=='post'){
		maple_save_meta_data($maple_new_meta_post_boxes, $post_id);
	}
}

/**
 * Saves the post meta for all types of posts.
 * @param $maple_new_meta_boxes the meta data array
 * @param $post_id the ID of the post
 */
function maple_save_meta_data($maple_new_meta_boxes, $post_id){

	if (isset($maple_new_meta_boxes) && !empty($maple_new_meta_boxes)){
			foreach($maple_new_meta_boxes as $meta_box) {
