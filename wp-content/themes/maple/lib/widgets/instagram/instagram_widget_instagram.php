...
class Maple_Instagram_Widget extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'instagram_widget', 'description' => esc_html__('Displays your latest Instagram photos.', 'maple'));
		parent::__construct(false, 'TW _ Instagram', $widget_ops);
	}
	
	function form($instance) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Instagram', 'maple' ), 'username' => '', 'link' => __( 'Follow Me!', 'maple' ), 'number' => 9, 'target' => '_self' ) );
		...
		if (!get_option('maple_insta_token') || get_option('maple_insta_token') == ""){
			echo '<p>Please go to Appearance > Maple Options > Social Networks > Instagram and click on Authorize Instagram to grant access to your feed.</p>';
		}
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'maple' ); ?>: <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of photos', 'maple' ); ?>: <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" /></label></p>
		
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>"><?php esc_html_e( 'Open links in', 'maple' ); ?>:</label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" class="widefat">
				<option value="_self" <?php selected( '_self', $target ) ?>><?php esc_html_e( 'Current window (_self)', 'maple' ); ?></option>
				<option value="_blank" <?php selected( '_blank', $target ) ?>><?php esc_html_e( 'New window (_blank)', 'maple' ); ?></option>
			</select>
		</p>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php esc_html_e( 'Link text', 'maple' ); ?>: <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'link' ) ); ?>" type="text" value="<?php echo esc_attr( $link ); ?>" /></label></p>
		<?php
	}
	...
	function treethemes_scrape_instagram($instance) {
		...
		$access_token = get_option('maple_insta_token', false);
		
		if (!$access_token){
			echo 'Please authorize Instagram on Appearance > Maple Options > Social Networks > Instagram and clicking on Authorize Instagram.';
			return;
		}
		...
		if (isset($response['data'])){
			...
			if (isset($feedinger['data'])){
				...
				if (get_option('maple_enable_instagram_grayscale') == "on") $imgclass .= ' maple_grayscale ';
			...
			echo 'Something went wrong. Please re-authorize Instagram on Appearance > Maple Options > Social Networks > Instagram and clicking on Authorize Instagram.';
			return;
		}
	}
	...
	function maple_get_user_items($user_id = null, $limit = 12, $next_max_id = null, $max_id = null) {
		
		$access_token = get_option('maple_insta_token');
		$shards = explode(".",$access_token);
		...
		return array_merge($instagram_feeds, $this->maple_get_user_items($user_id, $limit, $next_max_id, $max_id));
	}
	...
	function remote_get($url = null, $args = array(), $culprit = "") {
		if (!get_transient( 'maple_insta_transient'.$culprit )){
			$url = add_query_arg($args, trailingslashit($url));
			$response = $this->validate_response(wp_remote_get($url, array('timeout' => 29)));
			if (!isset($reponse['error'])) set_transient( 'maple_insta_transient'.$culprit, $response, 3600 );
		} else $response = get_transient( 'maple_insta_transient'.$culprit );
		return $response;
	}
	...
	function validate_response($json = null) {
		...
		if (!($response = json_decode(wp_remote_retrieve_body($json), true)) || 200 !== wp_remote_retrieve_response_code($json)) {
			...
			if (is_wp_error($json)) {
				...
				$response = array(
					'error' => 1,
					'message' => esc_html__('Unknow error occurred, please try again', 'maple')
				);
			}
		}
		return $response;
...
register_widget('Maple_Instagram_Widget');
