...
jQuery(document).ready(function($) {
	...
	window.maple_pageNum = parseInt(jQuery('#loader-startPage').html(),10) != 0 ? parseInt(jQuery('#loader-startPage').html(),10) : 1;
	...
	**/
	...
	if (nextLink != "empty" || prevLink != "empty"){
		if (jQuery('#reading_option').html() === "scroll" || jQuery('#reading_option').html() === "scrollauto"){
			if((parseInt(window.maple_pageNum, 10)+1) <= max) {
				...
				jQuery('.post-listing').each(function(){
					...
					if (jQuery(this).parents('.recentPosts_widget').length == 0){
						...
						if (jQuery(this).parents('.single').length == 0){
							var appendix = '<p id="pbd-alp-load-posts"><a href="javascript:;">'+ jQuery('#maple_load_more_posts_text').html() +'</a></p>';
							jQuery(this).parent()
								.append('<div style="position:relative;float:left;display:inline-block;width:100%;" class="pbd-alp-placeholder pbd-alp-placeholder-'+ (parseInt(window.maple_pageNum, 10)+1) +'"></div>')
								.append(appendix);			
						}
			...
			if (parseInt(window.maple_pageNum, 10) > 1){
				jQuery('.post-listing').each(function(){
		
					if (jQuery(this).parents('.recentPosts_widget').length == 0){
						
						if (jQuery(this).parents('.single').length == 0){
							var prependix = '<p id="pbd-alp-load-newer-posts" style="position: relative; height:50px;width:100%;text-align:center;"><a style=" position: relative; display:inline-block;" href="javascript:;">'+ jQuery('#maple_load_more_posts_text').html() +'</a></p>';
							jQuery(this).parent()
								.prepend('<div class="pbd-alp-placeholder-newer-'+ (parseInt(window.maple_pageNum, 10)-1) +'"></div>')
								.prepend(prependix);			
						}
			...
			if (jQuery('#pbd-alp-load-posts a').length){
				jQuery('#pbd-alp-load-posts a').click(function(e) {
					...
					if((window.maple_pageNum + window.maple_totalForward) <= max) {
					
						// Show that we're working.
						jQuery(this).html(''+jQuery('#maple_loading_posts_text').html()+'<img style="width:16px; height: 16px; margin-left: 5px; position: relative;" src="'+jQuery('#templatepath').html()+'/images/ajx_loading.gif">');
						
						window.first = true;
						
						jQuery('.pbd-alp-placeholder-'+ parseInt(window.maple_pageNum + window.maple_totalForward,10)).load( nextLink + ' article',
							function() {
								
								jQuery('.pbd-alp-placeholder-'+ parseInt(window.maple_pageNum + window.maple_totalForward,10)+' article').each(function(){
									/* masonry specifics */
								...
								if (window.first){
									var whereTo = jQuery('.pbd-alp-placeholder-'+ parseInt(window.maple_pageNum + window.maple_totalForward,10)).offset().top-100;
									window.first = false;
								...
								if (nextLink.indexOf('paged') < 0)
									nextLink = nextLink.replace(/\/page\/[0-9]?/, '/page/'+ parseInt(window.maple_pageNum + window.maple_totalForward,10)); /* links /page/x/ */
								else nextLink = nextLink.replace(/paged=[0-9]?/, "paged="+parseInt(window.maple_pageNum + window.maple_totalForward, 10)); /* links /?paged=x */
							
							
								// Add a new placeholder, for when user clicks again.
								jQuery('#pbd-alp-load-posts')
									.before('<div style="position:relative;float:left;width:100%;display:inline-block;" class="pbd-alp-placeholder pbd-alp-placeholder-'+ parseInt(window.maple_pageNum + window.maple_totalForward, 10) +'"></div>')
								
								// Update the button message.
								if((window.maple_pageNum + window.maple_totalForward ) <= max) {
									jQuery('#pbd-alp-load-posts a').text(jQuery('#maple_load_more_posts_text').html());
									...
									jQuery('#pbd-alp-load-posts a').text(jQuery('#maple_no_more_posts_text').html());
			...
			if (jQuery('#pbd-alp-load-newer-posts a').length){
				jQuery('#pbd-alp-load-newer-posts a').click(function(e) {
					...
					if((window.maple_pageNum + window.maple_totalBackward) > 0) {
					
						// Show that we're working.
						jQuery(this).html(''+jQuery('#maple_loading_posts_text').html()+'<img style="width:16px; height: 16px; margin-left: 5px; position: relative;" src="'+jQuery('#templatepath').html()+'/images/ajx_loading.gif">');
						jQuery('.pbd-alp-placeholder-newer-'+ (window.maple_pageNum + window.maple_totalBackward)).load( prevLink + ' article',
							function() {
		
								jQuery('.pbd-alp-placeholder-newer-'+ (window.maple_pageNum + window.maple_totalBackward)+' article').each(function(){
								...
								if ( prevLink.indexOf('paged') < 0 )
									prevLink = prevLink.replace(/\/page\/[0-9]?/, '/page/'+ (window.maple_pageNum + window.maple_totalBackward)); /* links /page/x/ */
								else prevLink = prevLink.replace(/paged=[0-9]?/, "paged="+(window.maple_pageNum + window.maple_totalBackward)); /* links /?paged=x */		
								
								
								// Add a new placeholder, for when user clicks again.
								jQuery('#pbd-alp-load-newer-posts')
									.after('<div class="pbd-alp-placeholder-newer-'+ parseInt((window.maple_pageNum + window.maple_totalBackward)) +'"></div>')
								
								// Update the button message.
								if((window.maple_pageNum + window.maple_totalBackward+1) > 1) {
									jQuery('#pbd-alp-load-newer-posts a').text(jQuery('#maple_load_more_posts_text').html());
								} else {
									jQuery('#pbd-alp-load-newer-posts a').text(jQuery('#maple_no_more_posts_text').html());
									var t=setTimeout(function(){
								...
								if (jQuery('.post-listing').hasClass('journal')){
									jQuery('.pbd-alp-placeholder-newer-'+ (window.maple_pageNum + window.maple_totalBackward)).remove();
								}
