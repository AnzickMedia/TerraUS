...
get_header(); maple_print_menu(); $maple_color_code = get_option("maple_style_color");
	...
	$height = "auto";
	$sidebar = maple_convert_to_class(get_option('maple_search_sidebars_available'));
	$sidebar = get_option('maple_search_sidebars_available');
	    	...
	    	<?php
				...
				<div class="container <?php echo esc_attr($originalalign); ?>" style="padding:<?php echo esc_attr($pagetitlepadding); ?> 15px;">
					...
					<?php
							...
							<h1 class="page_title" style="<?php echo esc_attr("color: #$tcolor; font-size: $tsize; font-family: '{$principalfont[0]}', sans-serif;font-weight: {$principalfont[1]}; ");?><?php if ($margintop != "") echo esc_attr("margin-top: ".intval($margintop,10)."px;"); ?>">
								<?php
									if (function_exists('icl_t')){
										echo wp_kses_post($counter->post_count . " " . sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Search results for', get_option('maple_search_results_text'))) . " &#8216;" . $_GET['s'] ."&#8217;");
									} else {
										echo wp_kses_post($counter->post_count . " " . sprintf(esc_html__("%s", "maple"), get_option("maple_search_results_text")) . " &#8216;" . $_GET['s'] ."&#8217;");
									}
								?>
							</h1>
							<?php
		    			...
		    			if ($showsectitle){
			    			if (is_string(get_option("maple_search_secondary_title")) && get_option("maple_search_secondary_title") != ""){
						    	?>
							    <h2 class="secondaryTitle" style="<?php echo esc_attr("color: #$stcolor; font-size: $stsize; line-height: $stsize; font-family: '{$secondaryfont[0]}'; font-weight:{$secondaryfont[1]}; margin-top:{$stmargin};");?>">
							    	<?php echo wp_kses_post(get_option("maple_search_secondary_title")); ?>
							    </h2>
					    		<?php
					    	}
		    			}
			...
			<div class="master_container" style="width: 100%;float: left;background-color: white;">
				<div class="container">
					<section class="page_content">
						<?php maple_print_search_results(); ?>
					</section>
				</div>
			</div>
			<?php
	...
	function maple_print_search_results(){
		global $maple_the_query;
			...
			<div class="navigation">
				<?php
					...
					if ($maple_reading_option != "paged" && $maple_reading_option != "dropdown"){ 
						$maple_the_query = new WP_Query();
						if (function_exists('icl_t')){
							next_posts_link('<div class="next-posts">&laquo; ' . sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Previous results', get_option('maple_previous_results'))).'</div>', $maple_the_query->max_num_pages);
							previous_posts_link('<div class="prev-posts">'.sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Next results', get_option('maple_next_results'))) . ' &raquo;</div>', $maple_the_query->max_num_pages);
						} else {
							next_posts_link('<div class="next-posts">&laquo; ' . sprintf(esc_html__("%s", "maple"), get_option("maple_previous_results")).'</div>', $maple_the_query->max_num_pages);
							previous_posts_link('<div class="prev-posts">'.sprintf(esc_html__("%s", "maple"), get_option("maple_next_results")) . ' &raquo;</div>', $maple_the_query->max_num_pages);
						}
					} else { 
						maple_wp_pagenavi();
					}
		...
		<div class="post-listing">
			<div class="pageTitle">
				<h2 class="hsearchtitle"><?php
					if (function_exists('icl_t')){
						echo sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'No results found.', get_option('maple_no_results_text')));
					} else {
						echo sprintf(esc_html__("%s", "maple"), get_option("maple_no_results_text"));
					}
				?></h2>
				<p class="titleSep"></p>
			</div>
