<?php
	...
	$maple_thisPostID = get_the_ID(); $maple_color_code = get_option("maple_style_color");

	if (get_post_meta($maple_thisPostID, "maple_enable_custom_pagetitle_options_value", true) == "no" || !get_post_meta($maple_thisPostID, "maple_enable_custom_pagetitle_options_value", true)){
		$type = get_option("maple_header_type");
		$thecolor = maple_hex2rgb(str_replace("__USE_THEME_MAIN_COLOR__", $maple_color_code, get_option("maple_header_color"))); 
		$opacity = intval(str_replace("%","",get_option("maple_header_opacity")))/100;
		$color = "rgba(".$thecolor[0].",".$thecolor[1].",".$thecolor[2].",".$opacity.")";
		$image = get_option("maple_header_image"); 
	$pattern = is_string(get_option("maple_header_pattern")) ? MAPLE_PATTERNS_URL.get_option("maple_header_pattern") : ""; 
		$custompattern = get_option("maple_header_custom_pattern"); 
		$margintop = get_option("maple_header_text_margin_top");	
		$banner = get_option("maple_banner_slider");
		$showtitle = get_option("maple_hide_pagetitle") == "on" ? true : false;
		$showsectitle = get_option("maple_hide_sec_pagetitle") == "on" ? true : false;
		$tcolor = get_option("maple".'_header_text_color');
		$tsize = intval(str_replace(" ", "", get_option("maple".'_header_text_size')),10)."px";
		$tfont = get_option("maple".'_header_text_font');
		$stcolor = get_option("maple".'_secondary_title_text_color');
		$stsize = intval(str_replace(" ", "", get_option("maple".'_secondary_title_text_size')),10)."px";
		$stfont = get_option("maple".'_secondary_title_font');
		$stmargin = intval(str_replace(" ", "", get_option("maple".'_header_sec_text_margin_top')),10)."px";
		$originalalign = get_option("maple_header_text_alignment");
		$pt_parallax = get_option("maple_pagetitle_image_parallax") == "on" ? true : false;
		$pt_overlay = get_option("maple_pagetitle_image_overlay") == "on" ? true : false;
		$pt_overlay_type = get_option("maple_pagetitle_overlay_type");
		$pt_overlay_the_color = maple_hex2rgb(str_replace("__USE_THEME_MAIN_COLOR__", $maple_color_code, get_option("maple_pagetitle_overlay_color")));
		$pt_overlay_pattern = (is_string(get_option("maple_pagetitle_overlay_pattern"))) ? MAPLE_PATTERNS_URL.get_option("maple_pagetitle_overlay_pattern") : "";
		$pt_overlay_opacity = intval(str_replace("%","",get_option("maple_pagetitle_overlay_opacity")))/100;
		$pt_overlay_color = "rgba(".$pt_overlay_the_color[0].",".$pt_overlay_the_color[1].",".$pt_overlay_the_color[2].",".$pt_overlay_opacity.")";
		$breadcrumbs = get_option("maple_breadcrumbs");
		$breadcrumbs_margintop = get_option('maple_breadcrumbs_text_margin_top');
		$pagetitlepadding = get_option('maple_page_title_padding');
	} else {
		$type = get_post_meta($maple_thisPostID, "maple_header_type_value", true);
		$thecolor = maple_hex2rgb(get_post_meta($maple_thisPostID, "maple_header_color_value", true)); 
		$opacity = intval(str_replace("%","",get_post_meta($maple_thisPostID, "maple_header_color_opacity_value", true)))/100;
		$color = "rgba(".$thecolor[0].",".$thecolor[1].",".$thecolor[2].",".$opacity.")";
		$image = get_post_meta($maple_thisPostID, "maple_header_image_value", true);
		$image = explode('|!|',$image);
		if (isset($image[1])) $image = explode('|*|',$image[1]);
		$image = $image[0];
		$pattern = MAPLE_PATTERNS_URL.get_post_meta($maple_thisPostID, "maple_header_pattern_value", true).".jpg";
		$custompattern = get_option("maple_header_custom_pattern_value"); 
		$margintop = get_post_meta($maple_thisPostID, "maple_header_text_margin_top_value", true);
		$banner = get_post_meta($maple_thisPostID, "maple_banner_slider_value", true);
		$showtitle = get_post_meta($maple_thisPostID, "maple_hide_pagetitle_value", true) == "yes" ? true : false;
		$showsectitle = get_post_meta($maple_thisPostID, "maple_hide_sec_pagetitle_value", true) == "yes" ? true : false;
		$tcolor = get_post_meta($maple_thisPostID, "maple_header_text_color_value", true);
		$tsize = intval(str_replace(" ", "", get_post_meta($maple_thisPostID, "maple_header_text_size_value", true)),10)."px";
		$tfont = get_post_meta($maple_thisPostID, "maple_header_text_font_value", true);
		$stcolor = get_post_meta($maple_thisPostID, "maple_secondary_title_text_color_value", true);
		$stsize = intval(str_replace(" ", "", get_post_meta($maple_thisPostID, "maple_secondary_title_text_size_value", true)),10)."px";
		$stfont = get_post_meta($maple_thisPostID, "maple_secondary_title_font_value", true);
		$stmargin = intval(str_replace(" ", "", get_post_meta($maple_thisPostID, "maple_header_secondary_text_margin_top_value", true)),10)."px";
		$originalalign = get_post_meta($maple_thisPostID, "maple_header_text_alignment_value", true);
		$pt_parallax = get_post_meta($maple_thisPostID, "maple_pagetitle_image_parallax_value", true) == "on" ? true : false;
		$pt_overlay = get_post_meta($maple_thisPostID, "maple_pagetitle_image_overlay_value", true) == "on" ? true : false;
		$pt_overlay_type = get_post_meta($maple_thisPostID, "maple_pagetitle_overlay_type_value", true);
		$pt_overlay_the_color = maple_hex2rgb(get_post_meta($maple_thisPostID, "maple_pagetitle_overlay_color_value", true));
		$pt_overlay_pattern = MAPLE_PATTERNS_URL.get_post_meta($maple_thisPostID, "maple_pagetitle_overlay_pattern_value", true).".jpg";
		$pt_overlay_opacity = intval(str_replace("%","",get_post_meta($maple_thisPostID, "maple_pagetitle_overlay_opacity_value", true)))/100;
		$pt_overlay_color = "rgba(".$pt_overlay_the_color[0].",".$pt_overlay_the_color[1].",".$pt_overlay_the_color[2].",".$pt_overlay_opacity.")";
		$breadcrumbs = get_post_meta($maple_thisPostID, "maple_enable_breadcrumbs_value", true) == "yes" ? "on" : "off";
		$breadcrumbs_margintop = intval(str_replace(" ", "", get_post_meta($maple_thisPostID, "maple_breadcrumbs_margin_top_value", true)),10)."px";
		$pagetitlepadding = intval(str_replace(" ", "", get_post_meta($maple_thisPostID, "maple_page_title_padding_value", true)),10)."px";
	}
	$height = "auto";
	...
	$singleLayout = get_post_meta($maple_thisPostID, 'singleLayout_value', true);
	if ($singleLayout == "default"){
		$singleLayout = get_option("maple_single_layout");
	}

	if (get_post_meta($maple_thisPostID, "portfolioType_value", true) != "other") {
		$pj_cols = " col-md-7";
		$ct_cols = " col-md-5";
		...
		<article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class(); ?> role="article">
				...
				<div class="projects_description">
					<div class="projects_media <?php echo esc_attr($singleLayout . $pj_cols); ?>">
						<?php
							...
							if (get_post_meta($maple_thisPostID, "portfolioType_value", true) == "image"){
								$output .= "<div id='p-slider-".esc_attr(get_the_ID())."' class='flexslider clearfix'><ul class='slides da-thumbs-plus'>";
								
								$sliderData = get_post_meta($maple_thisPostID, "sliderImages_value", true);
							...
							if (get_post_meta($maple_thisPostID, "portfolioType_value", true) == "video") {
								$videosType = get_post_meta($maple_thisPostID, "videoSource_value", true);
								if ($videosType != "embed"){
									$videos = get_post_meta($maple_thisPostID, "videoCode_value", true);
									$videos = preg_replace( '/\s+/', '', $videos );
									$vid = explode(",",$videos);
								}
								switch (get_post_meta($maple_thisPostID, "videoSource_value", true)){
									case "media":
									...
									case "vimeo":
										...
										break;
									case "embed":
										$output .= "<div class='embedded'>".get_post_meta($maple_thisPostID, "videoCode_value", true)."</div>";
										break;
								}
							}
					...
					<div class="content_container <?php echo esc_attr($ct_cols); ?>">
						<?php 
							...
							maple_content_shortcoder($content);
			...
			<?php
				if (get_option("maple_project_single_social_shares") == "on" && get_option('maple_project_single_socials') != ""){
				$proj_single_socials = explode(",",get_option('maple_project_single_socials'));
				?>
				<div class="share-buttons">
	                
		           <h5><?php 
			        	if (function_exists('icl_t')){
				        	echo sprintf(esc_html__("%s","maple"), icl_t( 'maple', 'SHARE THIS PROJECT', get_option('maple_share_proj_text'))); 
			        	} else {
				        	echo sprintf(esc_html__("%s","maple"), get_option("maple_share_proj_text")); 
			        	}
			        ?></h5>
		...
		<div class="projects_nav1">
			<?php 
				if (function_exists('icl_t')){
					previous_post_link( '<div class="nav-previous-nav1">%link</div>', sprintf(esc_html__("%s","maple"), icl_t( 'maple', 'Previous Project', get_option('maple_prev_single_proj')) )); 
					next_post_link( '<div class="nav-next-nav1">%link</div>', sprintf(esc_html__("%s", "maple"), icl_t( 'maple', 'Next Project', get_option('maple_next_single_proj')) )); 
				} else {
					previous_post_link( '<div class="nav-previous-nav1">%link</div>', sprintf(esc_html__("%s","maple"), get_option("maple_prev_single_proj") )); 
					next_post_link( '<div class="nav-next-nav1">%link</div>', sprintf(esc_html__("%s", "maple"), get_option("maple_next_single_proj") )); 
				}
			?>
		</div>
		...
		//image
		if (get_post_meta($maple_thisPostID, "portfolioType_value", true) == "image"){
			if (get_post_meta($maple_thisPostID, "custom_slider_opts_value", true) == "on"){
				$animation = get_post_meta($maple_thisPostID, "projs_flex_transition_value", true);
				$directionNav = get_post_meta($maple_thisPostID, "projs_flex_navigation_value", true);
				$slideshowSpeed = get_post_meta($maple_thisPostID, "projs_flex_slide_duration_value", true) != "" ? get_post_meta($maple_thisPostID, "projs_flex_slide_duration_value", true) : 3000;
				$pauseOnHover = get_post_meta($maple_thisPostID, "projs_flex_pause_hover_value", true);
				$controlNav = get_post_meta($maple_thisPostID, "projs_flex_controls_value", true);
				$slideshow = get_post_meta($maple_thisPostID, "projs_flex_autoplay_value", true);
				$height = get_post_meta($maple_thisPostID, "projs_flex_height_value", true);
				$animationDuration = get_post_meta($maple_thisPostID, "projs_flex_transition_duration_value", true) != "" ? get_post_meta($maple_thisPostID, "projs_flex_transition_duration_value", true) : 1000;
			} else {
				$animation = get_option("maple_projs_flex_transition");
				$directionNav = get_option("maple_projs_flex_navigation");
				$slideshowSpeed = get_option("maple_projs_flex_slide_duration") ? get_option("maple_projs_flex_slide_duration") : 3000;
				$pauseOnHover = get_option("maple_projs_flex_pause_hover");
				$controlNav = get_option("maple_projs_flex_controls");
				$slideshow = get_option("maple_projs_flex_autoplay");
				$height = get_option("maple_projs_flex_height");
				$animationDuration = get_option("maple_projs_flex_transition_duration") ? get_option("maple_projs_flex_transition_duration") : 1000;
			}
			...
			$maple_inline_script .= '
				if (jQuery("#p-slider-'.esc_html($maple_thisPostID).'").find("li").length > 1){
					jQuery("#p-slider-'.esc_html($maple_thisPostID).'").css("opacity",0).flexslider({
						animation: "'.esc_html($animation).'",
						slideDirection: "horizontal", 
						directionNav: "'.esc_html($directionNav).'",
						slideshowSpeed: '.esc_html($slideshowSpeed).',
						controlsContainer: "#p-slider-'.esc_html($maple_thisPostID).' .flex-viewport",
						pauseOnAction: false,
						...
						start: function(slider){
							...
							jQuery("#p-slider-'.esc_html($maple_thisPostID).'").css("opacity",1);
						}
					});
					jQuery("#p-slider-'.esc_html($maple_thisPostID).' ul.slides").css({"max-height":"'.esc_html($height).'"});
				} else {
					jQuery("#p-slider-'.esc_html($maple_thisPostID).'").find("ul li").css("display","block");
					jQuery("#p-slider-'.esc_html($maple_thisPostID).'").find("li a img").css("opacity",1);
					jQuery("#p-slider-'.esc_html($maple_thisPostID).'").find(".magnifier").bind("click", function(){
						jQuery("#p-slider-'.esc_html($maple_thisPostID).'").find("li a").trigger("click");
					});
				}
			';
		}
		//video
		if (get_post_meta($maple_thisPostID, "portfolioType_value", true) == "video") {
			if (get_post_meta($maple_thisPostID, "videoSource_value", true) != "embed" && get_post_meta($maple_thisPostID, "videoSource_value", true) != "media"){
				$maple_inline_script .= '
