<?php
/*
Plugin Name: Maple Custom Post Types
...
/* defines */
if (!defined('MAPLE_PORTFOLIO_POST_TYPE')){
	if (!defined('MAPLE_SHORTNAME')) define('MAPLE_SHORTNAME', 'maple');
	$portfolio_permalink = get_option(MAPLE_SHORTNAME."_portfolio_permalink");
	if (!get_option(MAPLE_SHORTNAME."_portfolio_permalink")) define("MAPLE_PORTFOLIO_POST_TYPE", "portfolio");
	else define("MAPLE_PORTFOLIO_POST_TYPE", get_option(MAPLE_SHORTNAME."_portfolio_permalink"));
}
if (!defined('MAPLE_TESTIMONIALS_POST_TYPE')){
	define("MAPLE_TESTIMONIALS_POST_TYPE", 'testimonials');
}

if (!defined('MAPLE_PLG_URL')){
	define('MAPLE_PLG_URL', plugin_dir_url(__FILE__) );
}
if (!defined('MAPLE_PLG_PATH')){
	define('MAPLE_PLG_PATH', plugin_dir_path(__FILE__) );
}

if (!defined('MAPLE_PLG_ACTIVE')){
	define('MAPLE_PLG_ACTIVE', true );
}
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'maple_register_portfolio_category');  //functions/portfolio.php
add_action('init', 'maple_register_portfolio_post_type');  //functions/portfolio.php
 ...
 */
if (!function_exists('maple_register_portfolio_category')){
    function maple_register_portfolio_category(){

        register_taxonomy("portfolio_category",
            array(MAPLE_PORTFOLIO_POST_TYPE),
 ...
 */
if (!function_exists('maple_register_portfolio_post_type')){
    function maple_register_portfolio_post_type() {
        $portfolio_permalink = get_option(MAPLE_SHORTNAME."_portfolio_permalink");
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Projects', 'portfolio name','maple'),
            'singular_name' => _x('Project Item', 'portfolio type singular name','maple'),
            'add_new' => __('Add New','maple'),
            'add_new_item' => __('Add New Item','maple'),
            'edit_item' => __('Edit Item','maple'),
            'new_item' => __('New Project Item','maple'),
            'view_item' => __('View Item','maple'),
            'search_items' => __('Search Project Items','maple'),
            'not_found' =>  __('No project items found','maple'),
            'not_found_in_trash' => __('No project items found in Trash','maple'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( MAPLE_PORTFOLIO_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'capability_type' => 'post',
                'menu_icon' => get_template_directory_uri() . '/images/maple_icons/projectsicon.png',
                'hierarchical' => false,
 ...
 */
if (!function_exists('maple_get_taxonomies')){
    function maple_get_taxonomies($type){
        $args = array(
            'type' => 'post',
            'orderby' => 'id',
            'order' => 'ASC',
 ...
 */
if (!function_exists('maple_get_taxonomy_slug')){
    function maple_get_taxonomy_slug($term_id){
        global $wpdb;
 ...
 */
if (!function_exists('maple_get_taxonomy_children')){
    function maple_get_taxonomy_children($type, $parent_id){
        global $wpdb;
...
if (!function_exists('maple_get_projects')){
    function maple_get_projects(){
        $proj = array();
        $args= array(
            'posts_per_page' =>-1,
            'post_type' => MAPLE_PORTFOLIO_POST_TYPE
        );
        query_posts($args);
 ...
 * ADD THE ACTIONS
 */
add_action('init', 'maple_register_testimonials_post_type');  //functions/testimonials.php


/**
 * Registers the portfolio custom type.
 */
if (!function_exists('maple_register_testimonials_post_type')){
    function maple_register_testimonials_post_type() {

        register_taxonomy("testimonials_category",
            array(MAPLE_TESTIMONIALS_POST_TYPE),
            array(	"hierarchical" => true,
                "label" => "Categories",
                "singular_label" => "Categories",
                "rewrite" => true,
        ...
        //the labels that will be used for the portfolio items
        $labels = array(
            'name' => _x('Testimonials', 'testimonials name','maple'),
            'singular_name' => _x('Testimonials Item', 'testimonials type singular name','maple'),
            'add_new' => __('Add New','maple'),
            'add_new_item' => __('Add New Item','maple'),
            'edit_item' => __('Edit Item','maple'),
            'new_item' => __('New Testimonials Item','maple'),
            'view_item' => __('View Item','maple'),
            'search_items' => __('Search Testimonials Items','maple'),
            'not_found' =>  __('No testimonials items found','maple'),
            'not_found_in_trash' => __('No testimonials items found in Trash','maple'),
            'parent_item_colon' => ''
        );

        //register the custom post type
        register_post_type( MAPLE_TESTIMONIALS_POST_TYPE,
            array( 'labels' => $labels,
                'public' => true,
                'show_ui' => true,
                'exclude_from_search' => true,
                'show_in_nav_menus' => false,
                'menu_icon' => get_template_directory_uri() . '/images/maple_icons/testicon.png',
                'capability_type' => 'post',
                'hierarchical' => false,
...
if (!function_exists('maple_init_cpt_plugin')){
    function maple_init_cpt_plugin(){
        maple_register_testimonials_post_type();
    }
}

if (!function_exists('maple_testimonials_categories_settings_field')){
	function maple_testimonials_categories_settings_field($settings, $value){
		...
		if (!count($tax_terms)){
			...
			if (count($tax_terms) > 1) $output .= "<label class='testimonial_categories'><input class='selectall' type='checkbox' name='categories[]' value='0' onchange=\"if(jQuery(this).is(':checked')){ jQuery(this).parent().siblings().children('input').attr('checked',true);jQuery(this).parent().siblings('input.".esc_js($settings['param_name'])."').val('-1');} else { jQuery(this).parent().siblings().children('input').attr('checked',false);jQuery(this).parent().siblings('input.".esc_js($settings['param_name'])."').val('0');}\" />".esc_html__('All','maple')."</label>";
			$value = explode(",",$value);
...
if (!function_exists('maple_fa_settings_field')){
	function maple_fa_settings_field($settings, $value) {
	   //$dependency = vc_generate_dependencies_attributes($settings);
		...
		$output = '<div class="maple_fa_block">'
		...
		foreach($icons as $i){
			...
			$output .= '" onclick="jQuery(this).closest(\'.edit_form_line\').find(\'input.maple_fa_field\').val(\''.esc_js($i).'\');jQuery(this).addClass(\'selected\').siblings().removeClass(\'selected\');"/>';
		}
		$output .= '</div>';
	   return $output;
	}
...
if (!class_exists('VCExtendAddonClass')){
	class VCExtendAddonClass {
	    ...
	    function __construct() {
	        ...
	        add_action( 'vc_before_init', array( $this, 'maple_integrateWithVC' ) );       
	        add_action( 'admin_enqueue_scripts', array( $this, 'treethemes_cpt_admin_scripts' ) );
	        
	        global $maple_testimonials_index, $maple_single_testimonial_index;
	        	$maple_testimonials_index = $maple_single_testimonial_index = 1;
	    }
	    ...
	    public function maple_integrateWithVC() {
		    
		    if (function_exists('add_shortcode')){
				add_shortcode( 'testimonials', array( $this, 'maple_renderTestimonials' ) );
	        }
	        ...
	        vc_map( array(
				'name' => esc_html__( 'Testimonials', 'maple' ),
				"category" => 'Shortcodes',
				'base' => 'testimonials',
				'is_container' => false,
				...
				'admin_enqueue_js' => MAPLE_PLG_URL . 'lib/vc_shortcodes/testimonials.js',
				'params' => array(
					array(
						'type' => 'dropdown',
						'heading' => esc_html__('Style', 'maple'),
						'param_name' => 'style_testimonials',
						'description' => esc_html__('3 Different Styles to choose from.','maple'),
						'value' => array(
							esc_html__( 'Style 1', 'maple' ) => 'style1',
							esc_html__( 'Style 2 (with scroller)', 'maple' ) => 'style2',
							esc_html__( 'Style 3 (single wide)', 'maple' ) => 'style3'
						),
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__('Animation Type','maple'),
						'param_name' => 'des_testimonials_flex_animation',
						'description' => esc_html__('Choose between Slide and Fade effects.','maple'),
						'value' => array(
							esc_html__( 'Slide', 'maple' ) => 'slide',
							esc_html__( 'Fade', 'maple' ) => 'fade'
						),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Slideshow?', 'maple' ),
						'param_name' => 'des_testimonials_flex_slideshow',
						'description' => esc_html__( 'Animate slider automatically.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes', esc_html__( 'No, thanks', 'maple' ) => 'no' )
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Slideshow Speed', 'maple' ),
						'param_name' => 'des_testimonials_flex_slideshow_speed',
						'description' => esc_html__( 'Set the speed of the slideshow cycling, in milliseconds.', 'maple' ),
						'value' => '3500'
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Animation Duration', 'maple' ),
						'param_name' => 'des_testimonials_flex_animation_duration',
						'description' => esc_html__( 'Set the speed of animations, in milliseconds.', 'maple' ),
						'value' => '1000'
					),
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in Desktop', 'maple' ),
						'param_name' => 'des_testimonials_flex_items_desktop',
						'description' => esc_html__( 'The number of visible items per slide in a desktop.', 'maple' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in a Small Desktop', 'maple' ),
						'param_name' => 'des_testimonials_flex_items_small_desktop',
						'description' => esc_html__( 'The number of visible items per slide in a small desktop.', 'maple' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in a Tablet', 'maple' ),
						'param_name' => 'des_testimonials_flex_items_tablet',
						'description' => esc_html__( 'The number of visible items per slide in a tablet.', 'maple' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Number of Items in Mobile', 'maple' ),
						'param_name' => 'des_testimonials_flex_items_mobile',
						'description' => esc_html__( 'The number of visible items per slide in a mobile.', 'maple' ),
						'value' => array('1'=>1,'2'=>2,'3'=>3),
					),
					/* endof new maple options */
					
					
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Direction Navigation', 'maple' ),
						'param_name' => 'des_testimonials_flex_direction_nav',
						'description' => esc_html__( 'Create navigation for previous/next navigation.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes', esc_html__( 'No, thanks', 'maple' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Navigation Style', 'maple' ),
						'param_name' => 'des_testimonials_flex_nav_style',
						'description' => esc_html__( 'Choose between Dark and Light style.', 'maple' ),
						'value' => array( esc_html__( 'Dark', 'maple' ) => 'dark', esc_html__( 'Light', 'maple' ) => 'light' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Control Navigation', 'maple' ),
						'param_name' => 'des_testimonials_flex_control_nav',
						'description' => esc_html__( 'Create navigation for paging control of each slide.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes', esc_html__( 'No, thanks', 'maple' ) => 'no' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Pause on Hover', 'maple' ),
						'param_name' => 'des_testimonials_flex_pause_on_hover',
						'description' => esc_html__( 'Pause the slideshow when hovering over slider, then resume when no longer hovering.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes', esc_html__( 'No, thanks', 'maple' ) => 'no' )
					),
					
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Testimonials Scroller Height', 'maple' ),
						'param_name' => 'des_testimonials_flex_height',
						'description' => esc_html__( 'The height of the testimonials scroller in pixels.', 'maple' ),
						'std' => '650'
					),
					/*endofflexoptions*/
					...
					array(
						...
						'heading' => esc_html__( 'Categories', 'maple' ),
						'param_name' => 'testimonials_cats',
						'description' => esc_html__( 'Choose one or more Categories', 'maple' )
					),
					array(
						'type' => 'textfield',
						'heading' => esc_html__( 'Number of Testimonials', 'maple' ),
						'param_name' => 'number',
						'description' => esc_html__( 'The number of testimonials. If set to 0 it will display all.', 'maple' )
					),
					/* new order options */
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order By', 'maple' ),
						'param_name' => 'maple_testimonials_orderby',
						'value' => array( esc_html__( 'Date', 'maple' ) => 'date', esc_html__( 'Author', 'maple' ) => 'author', esc_html__( 'Company', 'maple' ) => 'company' )
					),
					array(
						'type' => 'dropdown',
						'heading' => esc_html__( 'Order', 'maple' ),
						'param_name' => 'maple_testimonials_order',
						'value' => array( esc_html__( 'DESC', 'maple' ) => 'DESC', esc_html__( 'ASC', 'maple' ) => 'ASC' )
					),
					/* endof new order options */
					array(
						'type' => 'checkbox',
						'heading' => esc_html__( 'Hide author?', 'maple' ),
						'param_name' => 'hide_author',
						'description' => esc_html__( 'If selected, the author will not be displayed.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes' )
					),
					array(
						'type' => 'checkbox',
						'heading' => esc_html__( 'Hide company?', 'maple' ),
						'param_name' => 'hide_company',
						'description' => esc_html__( 'If selected, the company will not be displayed.', 'maple' ),
						'value' => array( esc_html__( 'Yes, please', 'maple' ) => 'yes' )
					)
				),
				'js_view' => 'VcTestimonialsView',
				'front_enqueue_js' => array(MAPLE_PLG_URL . "lib/vc_shortcodes/testimonials.js")
			) );
	    ...
	    */
	    ...
	    public function maple_renderTestimonials( $atts, $content = null ) {	
		  ...
		  global $maple_testimonials_index, $maple_single_testimonial_index;
			  $maple_testimonials_index = vc_is_inline() ? uniqid() : $maple_testimonials_index;
			  $maple_single_testimonial_index = vc_is_inline() ? uniqid() : $maple_single_testimonial_index;
		  $output = "";
		  ...
		  if ($style_testimonials === "style1"){
			  $output .= '<div id="testimonials-container-'.esc_attr($maple_testimonials_index).'" class="container testimonials '.esc_attr($style_testimonials).'"><div class="testimonial-box">'; 
			  
			  $first = true;
			  $maple_single_testimonial_index = 1;
			  $output .= '<ul class="testimonial-nav">';
			  foreach ($testimonials as $t){
				$output .= '<li><a href="#testimonial-'.esc_attr($maple_single_testimonial_index).'" ';
				if ($first){
					$first = false;
					$output .= ' class="active" ';
				}
				...
				$maple_single_testimonial_index++;
			  }
			  ...
			  foreach ($testimonials as $t){
				  	$active = ($maple_single_testimonial_index == 1) ? " active" : "";
					$output .= '<div class="testimonial'.esc_attr($active).'" id="testimonial-'.esc_attr($maple_single_testimonial_index).'">'.wp_kses_post($t->post_content);
			  ...
			  jQuery(document).ready(function(){
					var who = jQuery('#testimonials-container-".esc_js($maple_testimonials_index)."');
			...
			if ($style_testimonials == "style2"){
				$output .= '<div id="testimonials-slider-'.esc_attr($maple_testimonials_index).'" class="testimonials-style2 style-'.esc_attr($des_testimonials_flex_nav_style).'"><ul class="slides styled-list"'; 
			} else {
				$output .= '<div id="testimonials-slider-'.esc_attr($maple_testimonials_index).'" class="single-wide-testimonials testimonials-style2 style-'.esc_attr($des_testimonials_flex_nav_style).'"><ul class="slides styled-list"'; 
			}
			...
			<script type=\"text/javascript\">
				jQuery(document).ready(function(){
					...
					var who = jQuery('#testimonials-slider-".esc_js($maple_testimonials_index)." ul.slides');
...
if (!function_exists('maple_vc_before_init')){
	function maple_vc_before_init(){

		if (function_exists('vc_add_shortcode_param')){
			vc_add_shortcode_param('testimonials_cats', 'maple_testimonials_categories_settings_field');
			vc_add_shortcode_param('maple_fa', 'maple_fa_settings_field');	
		}
		...
		if (class_exists('WPBakeryVisualComposerAbstract')){
			if (!function_exists('treethemes_vc_library_cat_list')){
				function treethemes_vc_library_cat_list() {
					return array( __('All','maple') => 'all', 
						__('About','maple') => 'about', 
						__('FAQ','maple') => 'faq',
						__('Counters','maple') => 'counters',  
						__('Contact & Maps','maple') => 'contactforms', 						
						__('General','maple') => 'general',  
						__('Portfolio','maple') => 'portfolio',
						__('Pricing','maple') => 'pricing',
						__('Services','maple') => 'services',
						__('Testimonials','maple') => 'testimonials');
				}
			}
...
add_action('vc_before_init', 'maple_vc_before_init');

if (!function_exists('maple_add_increase_time')){
	function maple_add_increase_time(){
