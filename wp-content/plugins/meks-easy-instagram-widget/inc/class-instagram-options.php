...
class Meks_Instagram_Options {
	...
	/**
	 * Print connected facebook page insights - manage_pages permission
	 */
	// public function display_business_page_insights() {
		

	// 	$img 	  = $this->options['picture_url'];
	// 	$name 	  = $this->options['name'];
	// 	$username = $this->options['username'];

	// 	if ( empty($img) &&  empty($name) &&  empty($username) ) {
	// 		return false;
	// 	}

	// 	echo 	'<tr>
    //                 <th><label>Business Account Details</label</th>
	// 				<td>
	// 					<div class="mks_profile_wrapper">
	// 						<ul class="mks_profile_info">
	// 							<li><img class="mks_profile_img" src="'. esc_url( $img ) .'"></li>
	// 							<li><span>Facebook Page Name: <strong>'. esc_html( $name ) .'</strong></span></li>
	...
	public function get_business_data_api() {
		...
		if ( isset( $data->data[0]->instagram_business_account->id ) ) {
			...
			update_option( 'meks_instagram_settings', $this->options );
		}
